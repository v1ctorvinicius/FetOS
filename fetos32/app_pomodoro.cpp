...
void app_pomodoro_on_event(Event *e)
{
  ...
  if (current_state == POMO_ALARM)
  {
    if (gesture == GESTURE_LONG_PRESS)
    {
      ...
      if (current_state == POMO_BREAK_SHORT)
        time_left_ms = SHORT_BREAK_TIME;
      else if (current_state == POMO_BREAK_LONG)
        time_left_ms = LONG_BREAK_TIME;
      else
        time_left_ms = FOCUS_TIME;
  ...
  if (current_state == POMO_CONFIRM_RESET)
  {
    if (gesture == GESTURE_LONG_PRESS)
    {
  ...
  if (current_state == POMO_IDLE)
  {
    ...
    else if (gesture == GESTURE_TRIPLE_TAP)
    {
      ...
      current_state = POMO_CONFIRM_RESET;
    }
...
void app_pomodoro_render()
{
  ...
  if (current_state == POMO_ALARM)
  {
    ...
    if (blink)
      ui_center_text(oled, 20, "TIME'S UP!", 2);
  ...
  if (current_state == POMO_CONFIRM_RESET)
  {
    ...
    ui_center_text(oled, 0, "RESET SLOT?", 1);
    ui_center_text(oled, 20, slot_str, 2);
    ui_center_text(oled, 45, "Hold: Confirm", 1);
    ui_center_text(oled, 55, "Tap: Cancel", 1);
  ...
  if (current_state == POMO_IDLE)
  {
    ...
    if (dd > 0)
      snprintf(stats, sizeof(stats), "%lud %02lu:%02lu:%02lu", dd, hh, mm, ss);
    else if (hh > 0)
      snprintf(stats, sizeof(stats), "%lu:%02lu:%02lu", hh, mm, ss);
    else
      snprintf(stats, sizeof(stats), "%lu:%02lu", mm, ss);
  ...
  {
    if (current_state == POMO_FOCUS)
      ui_center_text(oled, 0, "FOCUS", 1);
    else if (current_state == POMO_BREAK_SHORT)
      ui_center_text(oled, 0, "SHORT BREAK", 1);
    else if (current_state == POMO_BREAK_LONG)
      ui_center_text(oled, 0, "LONG BREAK", 1);
    else if (current_state == POMO_PAUSED)
      ui_center_text(oled, 0, "PAUSED", 1);
